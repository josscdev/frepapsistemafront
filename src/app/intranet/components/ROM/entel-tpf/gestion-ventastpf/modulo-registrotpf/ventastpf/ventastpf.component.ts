...
import { EditarVentatpfComponent } from './editar-ventatpf/editar-ventatpf.component';
import { EditarVouchertpfComponent } from './editar-vouchertpf/editar-vouchertpf.component';
import { CrearVentatpfComponent } from './crear-ventatpf/crear-ventatpf.component';
import Swal from 'sweetalert2';
import { Jefetpf, SupervisorPDVtpf, Supervisortpf } from '../../../../../../models/entel-tpf/ventastpf/ventastpf';
...
import { Subproductotpf } from '../../../../../../models/entel-tpf/ventastpf/listasVentastpf';
...
@Component({
  ...
  templateUrl: './ventastpf.component.html',
  styleUrl: './ventastpf.component.css'
})
export class VentastpfComponent {
  ...
  openEdit(venta: any): void {
    const dialogRefedit = this.dialog.open(EditarVentatpfComponent, {
  ...
  openEditVoucher(venta: any): void {
    const dialogRefeditvoucher = this.dialog.open(EditarVouchertpfComponent, {
  ...
  openDialog(): void {
    ...
    if (this.perfil === 'HC') {
      const dialogRef = this.dialog.open(CrearVentatpfComponent, {
  ...
  jefesList: Jefetpf[] = [];
  supervisorList: Supervisortpf[] = [];
  pdvList: SupervisorPDVtpf[] = [];
  ...
  getSubproducto() {
    this.ventastpfService.getSubproducto(17).subscribe((res: Subproductotpf[]) => {
  ...
  buscar() {
    ...
    if (this.perfil === 'ADMIN') {
      this.ventasdetallelista$ =this.ventastpfService.getVentasAdmin(requestFilter);
      
    } else if (this.perfil === 'JV') {
      requestFilter.idjefe = localStorage.getItem('user') || '';

      this.ventasdetallelista$ =this.ventastpfService.getVentasJefe(requestFilter);
   
    } else if (this.perfil === 'SG') {
      requestFilter.idsuper = localStorage.getItem('user') || '';
      this.ventasdetallelista$ =this.ventastpfService.getVentasSuper(requestFilter);

    } else if (this.perfil === 'HC') {
      requestFilter.idpromotor = localStorage.getItem('user') || '';
      this.ventasdetallelista$ =this.ventastpfService.getVentasPromotor(requestFilter);
  ...
  eliminarventasdetalle(idventasdetalle: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.ventastpfService.getDeleteVentasDetalle(idventasdetalle, this.usuario).subscribe((res) => {
          console.log('res', res);
          if (res) {
            this.buscar();
