...
@Component({
  ...
  templateUrl: './modelostpf.component.html',
  styleUrl: './modelostpf.component.css'
})
export class ModelostpfComponent {
  manegementForm: UntypedFormGroup;
  modelosList: ModeloEquipotpf[] = [];
  filteredModelosList: ModeloEquipotpf[] = [];
  ...
  modelo: ModeloEquipotpf = new ModeloEquipotpf();
  ...
  ngOnInit(): void {
      ...
      this.modeloService.getModeloRomWeb(idemppaisnegcueDef!).subscribe({
        next: (data) => {
          if (data) {
            ...
            console.log('modelosList', this.modelosList)
    ...
    enableEditingModelo(item: ModeloEquipotpf) {
    ...
    saveChangesModelo(item: ModeloEquipotpf) {
    ...
    onDescriptionChangeModelo(event: Event, item: ModeloEquipotpf) {
    ...
    cancelEditingModelo(item: ModeloEquipotpf) {
      item.editing = false;
