...
import { Allocationtpf } from '../../../../../../models/entel-tpf/allocation-plantpf/allocationtpf';
import { EditarAllocationtpfComponent } from './editar-allocationtpf/editar-allocationtpf.component';
...
@Component({
  ...
  templateUrl: './allocationtpf.component.html',
  styleUrl: './allocationtpf.component.css'
})
export class AllocationtpfComponent implements OnInit {
  @ViewChild('paginator') paginator?: MatPaginator;
  readonly dialog = inject(MatDialog);
  perfil: string = "";

  allocationList: Allocationtpf[] = []
  filteredAllocationList: Allocationtpf[] = []
  ...
  openEdit(allocation: any): void {
    ...
    const dialogRefedit = this.dialog.open(EditarAllocationtpfComponent, {
      ...
      subscribe(res => {
        if (res?.length) {
          ...
          const peticiones = res.map((promotor: any) =>
            this.allocationtpfService.getRolUsuarioPDV(promotor.docusuario, this.idemppaisnegcue, periodo).pipe(
              map((puntosDeVenta) => ({
    ...
    if (!this.searchTerm.trim()) {
      ...
      this.filteredPuntosDeVentaPorPromotor = this.puntosDeVentaPorPromotor.filter((item) => {
        ...
        const matchesPuntosDeVenta = item.puntosDeVenta.some((pdv) =>
          pdv.nombrepdv.toLowerCase().includes(searchTermLower)
        );
  ...
  //       const month = fechainicio.getMonth() + 1; // Ajuste de índice (0-11 a 1-12)
  ...
  //         errorMessages.push(`El mes de fechainicio para docusuario ${item.docusuario} debe ser el mes: ${this.mes}(${this.numeroMes})`);
  //       }
  ...
  //       if (month !== this.numeroMes) { // Validación del mes actual
  //         errorMessages.push(`El mes de fechafin para docusuario ${item.docusuario} debe ser el mes: ${this.mes}(${this.numeroMes})`);
  //       }
  ...
  guardarRoles() {
    ...
    this.allocationtpfService.postRoles(this.dataAllocation).subscribe(res => {
