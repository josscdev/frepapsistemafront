...
import { Allocation, EstadoPromotor, Funcionalidad, PuntoVenta, TipoLicencia, TipoTrabajo } from '../../../../../../../models/entel-retail/allocation-plan/allocation';
...
@Component({
  selector: 'app-editar-allocation',
  standalone: true,
  imports: [
    MatDialogModule,
    MatTabsModule,
  ...
  templateUrl: './editar-allocation.component.html',
  styleUrl: './editar-allocation.component.css'
})
export class EditarAllocationComponent implements OnInit {
  readonly dialogRef = inject(MatDialogRef<EditarAllocationComponent>);
  editForm: FormGroup;

  puntoVentaList: PuntoVenta[] = [];
  estadoPromotorList: EstadoPromotor[] = [];
  funcionalidadList: Funcionalidad[] = [];
  tipoLicenciaList: TipoLicencia[] = [];
  ...
  constructor(
    ...
    // console.log('this.numMes', this.numMes)
  ...
  createRoleFormGroup(role: any, index: number): FormGroup {
    ...
    //   this.botonesGuardar[this.getRolesFormArray().length - 1] = formGroup.dirty;
    //   console.log('formGroup.dirty',formGroup.dirty)
    //   console.log('this.botonesGuardar[this.getRolesFormArray().length - 1]'
    //     ,this.botonesGuardar[this.getRolesFormArray().length - 1])
    //   console.log('this.getRolesFormArray().length - 1',this.getRolesFormArray().length - 1)
    // });
  ...
  getPdv() {
    this.allocationService.getPdv(this.idemppaisnegcue).subscribe(res => {
      if (res !== null) {
  ...
  getEstadoPromotor() {
    this.allocationService.getTipoEstado(this.idemppaisnegcue).subscribe(res => {
      if (res !== null) {
  ...
  getFuncionalidad() {
    this.allocationService.getTipoFuncionalidad(this.idemppaisnegcue).subscribe(res => {
      if (res !== null) {
  ...
  getTipoLicencia() { //combobox Motivo
    this.allocationService.getTipoLicencia(this.idemppaisnegcue).subscribe(res => {
      if (res !== null) {
  ...
  getTipoTrabajo() {
    this.allocationService.getTipoTrabajo(this.idemppaisnegcue).subscribe(res => {
      if (res !== null) {
  ...
  saveAllocation() {
    ...
    const request: Allocation[] = this.listaRolesAbd;
    ...
    // this.allocationService.putRoles(request).subscribe(res => {
    //   if (res) {
  ...
  removeOneRole(index: number) {
    ...
    console.log('Debe entrar', roleFormGroup)
    // Remueve el formulario del FormArray para actualizar la vista
    if (idrolValue === 0) {
      this.getRolesFormArray().removeAt(index);
    } else {
