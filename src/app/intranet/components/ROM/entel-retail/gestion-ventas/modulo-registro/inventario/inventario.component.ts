...
import { InsertarInventario, InsertarInventarioDetalle, InsertarSoloInventarioDetalle, ListarInventario } from './models/inventario';
import { MatDialog, MatDialogConfig } from '@angular/material/dialog';
import { EditarInventarioComponent } from './editar-inventario/editar-inventario.component';
import { DetalleFaltantesOracleComponent } from './detalle-faltantes-oracle/detalle-faltantes-oracle.component';
...
export class InventarioComponent implements OnInit {
  ...
  constructor(
    ...
    private inventarioService: InventarioService, // Cambia esto por el servicio correcto
  ...
  createForm(): UntypedFormGroup {
    ...
    return this.fb.group({
      ...
      idpdv: new FormControl(Number(localStorage.getItem('idpdv')))
  ...
  abrir() {
    if (Number(localStorage.getItem('idpdv')) === 0 && this.perfil !== 'ADMIN') {
      Swal.fire({
        icon: 'warning',
        title: 'Alerta',
        text: 'No tiene punto de venta asignado',
      })
      return;
    }
    ...
    const renderHtml = () => `
      <input id="imei-input" class="swal2-input" maxlength="16" placeholder="Escanee IMEI">
  ...
  //       <table class="table table-sm table-bordered">
  //         <thead>
  //           <tr><th>#</th><th>IMEI</th><th>Acción</th></tr>
  //         </thead>
  //         <tbody id="imei-table-body">
  //           ${imeisTemp.map((imei, i) => `
  //             <tr data-index="${i}">
  //               <td>${i + 1}</td>
  //               <td>${imei}</td>
  //               <td>
  //                 <button type="button" class="btn btn-danger btn-eliminar-imei" title="Eliminar IMEI"
  ...
  //         const nuevoImei = input.value;
  //         if (nuevoImei.length === 15) {
  //           if (imeisTemp.includes(nuevoImei)) {
  //             errorDiv.textContent = `El IMEI ${nuevoImei} ya fue registrado.`;
  //             input.value = '';
  //             return;
  //           }

  //           imeisTemp.push(nuevoImei);
  //           input.value = '';
  //           errorDiv.textContent = '';
  ...
  //         const tbody = document.getElementById('imei-table-body');
  //         if (tbody) {
  //           tbody.innerHTML = imeisTemp.map((imei, i) =>
  //             `<tr data-index="${i}">
  //               <td>${i + 1}</td>
  //               <td>${imei}</td>
  //               <td>
  ...
  //           const botones = tbody.querySelectorAll('.btn-eliminar-imei');
  //           botones.forEach((btn, idx) => {
  //             btn.addEventListener('click', function () {
  //               imeisTemp.splice(idx, 1);
  //               actualizarTabla();
  //             });
  //           });
  //         }
  ...
  //       // Inicializar eventos de eliminar si hay IMEIs precargados
  //       actualizarTabla();
  //     }
  //   }).then(result => {
  //     if (result.isConfirmed && result.value) {
  ...
  editarInventario(item: ListarInventario) {
    ...
    const dialogRef = this._dialog.open(EditarInventarioComponent, config);
  ...
  verFaltantes() {
    ...
    const dialogRef = this._dialog.open(DetalleFaltantesOracleComponent, config);
