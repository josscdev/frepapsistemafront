...
import { Bundle, Modelo, Operador, Planes, Subproducto, TipoAccesorio, TipoBiometria, TipoDocumento, TipoEquipo, TipoEtiqueta, TipoPago, TipoSeguro } from '../../../../../../../models/entel-retail/ventas/listasVentas';
...
export class CrearVentaComponent implements OnInit {
  readonly dialogRef = inject(MatDialogRef<CrearVentaComponent>);
  readonly data = inject<DialogData>(MAT_DIALOG_DATA);
  readonly animal = model(this.data.animal);
  botonBloqueado: any = false;
  tipoDocumentoList: TipoDocumento[] = [];
  tipoBiometriaList: TipoBiometria[] = [];
  ...
  getTipoDocumento() {
    this.ventasService.getTipoDocumento(this.idemppaisnegcue).subscribe(res => {
  ...
  getTipoBiometria() {
    this.ventasService.getTipoBiometria(this.idemppaisnegcue).subscribe(res => {
  ...
  getBundle() {
    this.ventasService.getBundle(this.idemppaisnegcue).subscribe(res => {
      ...
      if (res !== null) {
        this.bundleList = res.filter((x: Bundle) => x.estado === 1);
      }
    })
  }
  ...
  //       }) as ListaImeis);
  //       // Trigger the first filtering to initialize the list
  //       //this.idIMEIequipoControl.setValue('');
  //     }
  //   });
  // }
  ...
  changeTipoDocumento(event: any) {
    ...
    if (tipodocumento && tipodocumento.idtipodocumento) {
      ...
      if (idtipodocumentoSeleccionado && idtipodocumentoSeleccionado.nombretipodocumento.includes('DNI')
        || idtipodocumentoSeleccionado && idtipodocumentoSeleccionado.nombretipodocumento.includes('RUC')) {//DIGITAL
      ...
      if (idtipodocumentoSeleccionado && idtipodocumentoSeleccionado.nombretipodocumento.includes('C.E')
        || idtipodocumentoSeleccionado && idtipodocumentoSeleccionado.nombretipodocumento.includes('PASAPORTE')) {//MANUAL
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      reader.onload = () => {
        ...
        this.ventasService.getDataReaderCode(this.base64File).subscribe(res => {
          console.log('res', res);
  ...
  onSubmit(): void {
    ...
    if (this.ventaForm.valid) {
      ...
      const nuevoDetalle: VentasDetalle = {
  ...
  eliminarVenta(detalle: VentasDetalle): void {
  ...
  changeInputs(event: any) {
    ...
    if (subproducto && subproducto.id) {
      ...
      if (subproductoSeleccionado && subproductoSeleccionado.name.includes('PORTA')) {
        ...
        console.log('upselling1', subproductoSeleccionado);
  ...
  saveVentas() {
    ...
    this.ventasService.postVentas(nuevaVenta).subscribe(res => {
      if (res !== null) {
        ...
        if (res.success) {
          ...
          //Guardar Voucher
          if (isVoucher) {
            const requestUploadVoucher: UploadImageRequest = {
              Base64Image: this.base64File,
              VoucherName: res.nombrevoucher
            }

            this.ventasService.uploadVoucherRetail(requestUploadVoucher).subscribe(result => {
              if (result !== null) {
                if (result.status === 'OK') {
                  console.log('Resultado Subida S3', result);
                  const requestUpdateNombreVoucher: ActualizarNombreVoucherRequest = {
                    idventa: res.idventa,
                    nombrevoucher: result.nombreVoucher
                  }
                  this.ventasService.updateNameVoucherRetail(requestUpdateNombreVoucher).subscribe(response => {
  ...
  // saveVentas(): Promise<boolean> {
  //   return new Promise((resolve, reject) => {
  //     const startDate = this.ventaForm.get('startdate')?.value;
  //     const formattedStartDate = this.formatDateSave(startDate);

  //     let isVoucher = this.base64File ? true : false;
  ...
  //               this.ventasService.updateNameVoucherRetail(requestUpdateNombreVoucher).subscribe(() => {
  //                 Swal.fire({
  //                   title: 'Cargando...',
  ...
  //                     Swal.showLoading();
  //                   },
  //                   timer: 0 // Espera adicional opcional
  //                 }).then(() => {
  //                   resolve(true); // Completa la promesa con éxito
  //                   this.dialogRef.close();
  //                 });
  //               });
  //             } else {
  ...
  //             confirmButtonText: 'Ok'
  //           });
  //           resolve(true); // Completa con éxito
  //           this.dialogRef.close();
  //         }
  //       } else {
  //         Swal.fire({
