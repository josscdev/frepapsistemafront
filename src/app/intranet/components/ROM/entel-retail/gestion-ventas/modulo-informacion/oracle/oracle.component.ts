...
import { FiltroInsertarConciliar, InsertarOracleLogistica, MatchResumenDto } from './models/oracle';
...
export class OracleComponent {
  ...
  private readonly _AllocationService = inject(AllocationService);
  ...
  lConciliaciones: MatchResumenDto[] = [];
  lFiltradaConciliaciones: MatchResumenDto[] = [];
  ...
  async conciliarOracle() {
    ...
    try {
      ...
      if (response) {
        ...
        this.lConciliaciones = uniqueByNombrePdv;
        this.aplicarFiltro();
  ...
  aplicarFiltro() {
    ...
    this.lFiltradaConciliaciones = this.lConciliaciones.filter(item =>
  ...
  detalleConciliacion(item: any) {
    ...
    const dialogRef = this._dialog.open(DetalleConciliacionComponent, config);
