...
import { ActualizarNombreVoucherRequest, UploadImageRequest, Ventas } from '../../../../../../../models/entel-retail/ventas/ventas';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';

@Component({
  selector: 'app-editar-voucher',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    CommonModule,
  ...
  templateUrl: './editar-voucher.component.html',
  styleUrl: './editar-voucher.component.css'
})
export class EditarVoucherComponent {
  readonly dialogRef = inject(MatDialogRef<EditarVoucherComponent>);
  ...
  constructor(
    ...
    console.log('nombre voucher',nombrevoucher)
    if (nombrevoucher) {
      const request: ActualizarNombreVoucherRequest = {
        nombrevoucher: nombrevoucher
      };
      this.isLoading = true; // Aquí se establece isLoading en true
      this.ventasService.getUrlPrefirmada(request).subscribe(result => {
  ...
  saveBoleta() {
    ...
    if (isVoucher) {
      ...
      this.ventasService.uploadVoucherRetail(requestUploadVoucher).subscribe(res => {
        if (res.status === 'OK') {
          console.log('Resultado Subida S3', res);

          this.ventasService.updateVoucherVentas(voucherVenta).subscribe(response => {
