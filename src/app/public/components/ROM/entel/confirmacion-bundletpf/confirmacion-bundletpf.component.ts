...
import { ValidacionBundlestpfService } from '../../../../services/ROM/entel/validacion-bundles/validacion-bundlestpf.service';
...
@Component({
  selector: 'app-confirmacion-bundletpf',
  standalone: true,
  imports: [
    FormsModule,
  ...
  templateUrl: './confirmacion-bundletpf.component.html',
  styleUrl: './confirmacion-bundletpf.component.css'
})
export class ConfirmacionBundletpfComponent implements OnInit, AfterViewInit  {
  ...
  constructor(
    ...
    private validacionBundlesService: ValidacionBundlestpfService,
  ...
  async guardarBundle(callback: () => void) {
    if (this.validacionBundle.nombrebundle && this.validacionBundle.nombrebundle.includes('OBSEQUIOS') &&
    ...
    this.validacionBundle.usuariocreacion = this.validacionBundle.docpromotorasesor;
    
    if (this.idemppaisnegcue === 0) {
      const Toast = Swal.mixin({
        toast: true,
        position: "bottom-end",
        showConfirmButton: false,
      ...
      Toast.fire({
        icon: "error",
        showCloseButton: true,
        title: "Error de Servidor, reinicie la página"
      });
      return;
    }
    
    this.validacionBundle.idemppaisnegcue = this.idemppaisnegcue;
    ...
    this.validacionBundlesService.postBundlesFirma(this.validacionBundle).subscribe(async res => {
      ...
      if (res.mensaje === 'OK') {
        ...
        this.nameCodigo = res.codigoBundle;
            ...
            async (response: any) => {
              if (response.status !== 'OK' || response.status === undefined || response.status === null) {
                ...
                if (response && response.pdfBase64) {
                  ...
                  try {
                    const res = await this.validacionBundlesService.validarSubidaS3(this.validacionBundle.idventasdetalle ?? 0).toPromise();
  ...
  getDocumentDefinition(): any {
    ...
    const fechaVentaFormateada = this.datePipe.transform(this.validacionBundle.fechaoperacion, 'dd/MM/yyyy');
    ...
    return {
      content: [
        ...
        { text: 'Agradecemos su preferencia con el siguiente premio: ' + this.validacionBundle.nombrebundle, style: 'paragraph' },
        ...
        { text: 'Datos Cliente', style: 'headerTable' },
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'DNI Cliente:', bold: true }, this.validacionBundle.doccliente],
              [{ text: 'N° Celular de Contrato:', bold: true }, this.validacionBundle.numcelularcontrato],
            ],
        ...
        { text: 'Datos de la Venta', style: 'headerTable' },
        {
          ...
          table: {
            ...
            body: [
              ...
              [{ text: 'DNI Promotor:', bold: true }, this.validacionBundle.docpromotorasesor],
              [{ text: 'Punto de Venta:', bold: true }, this.validacionBundle.nombrepdv],
              [{ text: 'Producto:', bold: true }, this.validacionBundle.nombresubproducto],
              [{ text: 'N° Orden:', bold: true }, this.validacionBundle.numeroorden]
            ],
            // Grosor y color de las líneas de la tabla
            layout: {
              hLineWidth: function (i: any, node: any) {
        ...
        { text: 'Datos Premio', style: 'headerTable' },
        {
          ...
          table: {
            ...
            body: [
              [{ text: 'Premio:', bold: true }, this.validacionBundle.nombrebundle],
          ...
          columns: [
            {
              stack: [
                ...
                { text: 'DNI Cliente: ' + this.validacionBundle.doccliente, style: 'textoFirma' }
              ],
              width: '50%',
              alignment: 'center'
            },
  ...
  getDataById(id: number): void {
    this.validacionBundlesService.getById(id).subscribe(data => {
      ...
      this.idemppaisnegcue = Number(this.validacionBundle.idemppaisnegcue ?? 0);
      localStorage.setItem('idemppaisnegcue', this.idemppaisnegcue.toString());
      ...
      this.bundleForm.patchValue({
        ...
        doccliente: data.doccliente,
        nombresubproducto: data.nombresubproducto,
        nombrebundle: data.nombrebundle,
      });
    });
  }
  ...
  createFormBundle(): UntypedFormGroup {
    return this.fb.group({
      ...
      doccliente: new FormControl('', Validators.compose([
        Validators.required,
      ])),
      nombresubproducto: new FormControl('', Validators.compose([
        Validators.required,
      ])),
      ...
      numcelularcontrato: new FormControl('', Validators.compose([
        Validators.required,
      ])),
  ...
  postCodigoAutenticacion() {
    ...
    let codigoauthbundle = this.bundleForm.get('codigoAutenticacion')?.value;

    console.log('codigoauthbundle', codigoauthbundle)

    const idventasdetalle = this.validacionBundle.idventasdetalle;

    if (idventasdetalle !== undefined && codigoauthbundle) {
      this.validacionBundlesService.ValidarCodigoAuthBundle(idventasdetalle, codigoauthbundle).subscribe(res => {
        if (res.mensaje === 'OK') {
          this.isKeyIn = true;
          this.isValidated = true;
          Swal.fire({
